...
for (const key in EVENT_COLOR_HEX) {
    if (EVENT_COLOR_HEX.hasOwnProperty(key)) {
        const hexValue = EVENT_COLOR_HEX[key];
...
function checkEventExists(eventData) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const response = yield fetch(url, { method: 'GET', cache: 'no-cache' });
            if (!response.ok) {
        ...
        switch (msg.type) {
            ...
            case 'create-event':
                ...
                try {
                    ...
                    const newFrame = yield createEventFrame(eventId, msg.eventType || (msg.eventData && msg.eventData.event_name) || 'interaction');
                    if (newFrame) {
            ...
            case 'insert-existing-event':
                ...
                const insertedFrame = yield createEventFrame(msg.eventId, msg.eventType);
                if (insertedFrame) {
            ...
            case 'list-events':
                ...
                try {
                    const events = yield getExistingEvents();
                    figma.ui.postMessage({
                        type: 'events-list',
                        events: events || []
                    });
            ...
            case 'add-existing-event':
                ...
                const frame = yield createEventFrame(msg.eventId.toString(), msg.eventType || 'interaction');
                if (frame) {
            ...
            case 'use-existing-event':
                ...
                try {
                    ...
                    const existingFrame = yield createEventFrame(msg.event.event_id.toString(), msg.event.event_name || 'interaction');
                    if (existingFrame) {
